...
const REASONING_ITEMS = [
  { icon: '🥩', label: 'Protein',  key: 'protein',  color: '#4F46E5', bg: '#EEF2FF' },
  { icon: '💧', label: 'Water',    key: 'water',    color: '#059669', bg: '#ECFDF5' },
  { icon: '🔥', label: 'Calories', key: 'calories', color: '#D97706', bg: '#FFFBEB' },
  { icon: '⏱️', label: 'Fasting',  key: 'fasting',  color: '#7C3AED', bg: '#F5F3FF' },
]
...
export default function SetupGoalsPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#EEF2F8', padding: '32px 16px', fontFamily: 'system-ui,sans-serif' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                {GOAL_OPTIONS.map(o => (
                  <button key={o.value} onClick={() => toggleGoal(o.value)} style={{
                    ...
                    {userGoals.includes(o.value) && (
                      <span style={{ position: 'absolute', top: 4, left: 6, fontSize: 10 }}>✓</span>
                    )}
                    {o.label}
                  </button>
                ))}
          ...
          <>
            {aiLoading ? (
              <div style={{ ...card({ textAlign: 'center', padding: '56px 20px' }) }}>
                ...
                <div style={{ fontSize: 12, color: '#9CA3AF' }}>Mifflin-St Jeor · Boer LBM · ISSN 2022 · Ayurveda Chronobiology</div>
              </div>
            ) : (
              <>
                {/* AI Recommendation Card */}
                {reasoning && (
                  <div dir="ltr" style={{ ...card({ background: 'linear-gradient(135deg,#EEF2FF,#F5F3FF)', border: '1px solid #C7D2FE', padding: '16px' }), dir: 'ltr' }}>
                    {/* Header */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                      <span style={{ fontSize: 16 }}>🤖</span>
                      <span style={{ fontSize: 13, color: '#4F46E5', fontWeight: 700 }}>AI Recommendation</span>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6, marginBottom: 14 }}>
                      {[
                        ...
                        { label: 'Bio',      value: profile?.bio_constitution ?? '—' },
                        { label: 'Blood',    value: profile?.blood_type ?? '—' },
                        { label: 'Activity', value: activityLevel },
                        { label: 'Work',     value: workType },
                        { label: 'Goals',    value: userGoals.length + ' selected' },
                      ].map(item => (
                        <div key={item.label} style={{
                          background: 'rgba(255,255,255,0.7)', borderRadius: 8,
                          padding: '6px 4px', textAlign: 'center'
                        }}>
                          <div style={{ fontSize: 9, color: '#818CF8', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.3 }}>{item.label}</div>
                          <div style={{ fontSize: 11, color: '#3730A3', fontWeight: 700, marginTop: 2 }}>{item.value}</div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {REASONING_ITEMS.map(item => (
                        <div key={item.key} style={{
                          ...
                          <div style={{ fontSize: 11, color: item.color, fontWeight: 700, marginBottom: 3 }}>
                            {item.icon} {item.label}
                          </div>
                          <div style={{ fontSize: 11, color: '#374151', lineHeight: 1.5 }}>
                            {(reasoning as any)[item.key]}
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {[
                  ...
                  <div key={key} style={card()}>
                    ...
                    <div dir="ltr" style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <input dir="ltr" type="range" min={min} max={max} step={step} value={goals[key] as number}
                        onChange={e => setGoals({ ...goals, [key]: Number(e.target.value) })}
                        style={{ flex: 1, accentColor: color }} />
                      <span style={{ fontWeight: 700, color, minWidth: 72, textAlign: 'right' }}>{goals[key]} {unit}</span>
                    </div>
                  </div>
                ))}
                ...
                <div style={card()}>
                  ...
                  <p style={{ fontSize: 11, color: '#9CA3AF', marginTop: 8, textAlign: 'center' }}>
                    Eating starts: {goals.eating_window_start} — ends after {24 - goals.fasting_window_hours}h
                  </p>
                </div>
